...
function moveToFloor(floor, initialize) {
  ...
  var trackFloor = setInterval(function () {
    ...
    var checkFloor = setInterval(function () {
      ...
      console.log('------');
      console.log(floor); // 5
      console.log(getNextFloor(toNumber(floorTwo))); // 4
      if (floor !== getNextFloor(toNumber(floorTwo))) { // queue[0]
        queueActive = true;
        clearInterval(trackFloor);
        clearInterval(checkFloor);
        moveToFloor(getNextFloor(toNumber(floorTwo))); // goToFloor??  ||  queue[0]
        // removeCurrentFloor();
      }
    }, 10);
...
function removeCurrentFloor() {
  queue.splice(queue.indexOf(currentFloor), 1);
}


// TODO: styles
// TODO: elevator on down skipps second pressed
